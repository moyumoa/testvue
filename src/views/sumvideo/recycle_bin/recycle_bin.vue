<template>
  <div class="recycle_page sumvideo_page">
    <!-- <down-load-client class="download_client" v-if="systemVersion != 3"></down-load-client> -->

    <div class="page_top">
      <div class="top_left">
        <div class="left_title">回收站</div>
        <div class="left_span">此处存放30天内删除模板、过期后自动清理</div>
      </div>
      <div class="top_right">
        <el-checkbox
          v-model="$data.isAll"
          ...
          >
            ...
            >
              <div class="video_cover_box">
                <img
                  v-if="item.creativeVo.videoStatus == 2 && item.creativeVo.status != 1"
                  class="video_cover"
                  :src="item.fileCover || defaultCover"
                  alt=""
                />
                <!-- 合成失败 -->
                ...
                <!-- 草稿 -->
                <div class="mask" v-if="item.creativeVo.status === 1">
                  <div class="wrapper">
                    <img
...
import basicinfoDOM from '../creative_mould/components/basicInfo_dialog.vue'
...
const openDetail = throttle((item, index) => {
  if (item.creativeVo) {
    item.creativeVo.materialPath = item.creativeVo.videoUrl
    basicInfoDialog.id = item.id
    basicInfoDialog.info = item.creativeVo
...
function getWidth() {
  listWidth.value = getCWidth() - 32 - 34 - 7
  init()
}
...
function init() {
  ...
  cLen.value = Math.floor((getCWidth() - 32 - 34 - 96) / 206)
