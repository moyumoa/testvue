      ...
      <div v-if="infoDialog.type == 'upload'">
        ...
        <div class="batch-label">
          ...
          <div style="width: 696px">
            <batchMarking :origin="infoDialog.origin" @choseOptions="choseOptions" />
          </div>
        </div>
        <div class="batch-label" style="margin-top: 0">
        ...
        <div class="last-num">
          ...
          <div class="num-r" style="width: 696px">
            ...
            <div class="num-r-bottom">
              上传批次号便于查找批量上传的同批{{
                origin === 'finishProductStore' ? '成片' : '素材'
              }}，批量进行打标、删除操作
            </div>
          </div>
        </div>
  ...
  infoDialog: {
    ...
    default: () => {
      return {
        ...
        littleTips: '', // 上传按钮的提示
        origin: '', // 来源 素材库finishProductStore
        batchItem: -1, // 1去除选中素材标签 2去除所有素材标签 3选中素材打标 4素有素材打标 5删除选中素材 6删除所有素材
...
onMounted(() => {
  // 获取上传批次号
  fetBatchNumInit()
  tipsType.value = origin.value === 'finishProductStore' ? '成片' : '素材'
})
...
function handleBeforeUpload(file, fileList) {
  ...
  if (!allowedTypes.includes(suffix.toLowerCase())) {
    ...
    message({
      ...
      message: '单个图片不超过15M，单个视频不超过500M且单次上传不超过50个成片'
    })
      ...
      putObject(res.data, option.file).then(resPut => {
        if (resPut) {
          ...
          if (fileLoading.value && namePonitab[0] === clickTime) {
            ...
            // }
            // 素材库没有入库概念不调这个接口，上边的步骤（上传阿里云成功）就已经算添加成功
            if (origin.value !== 'finishProductStore') {
...
async function uploadFile(uploadForm, option) {
  ...
  await batchUpload(arr)
    .then(resUpload => {
      ...
      if (!audioInfo.hasAudio && initFiletype(resUpload.data[0].fileFormat) === 'radio') {
...
const saveFileUpload = throttle(function () {
  ...
  if (successFileList.value.length === 0) {
    saveLoading.value = false
    message.warning(`未上传${tipsType.value || '素材'}`)
    return
  }
  if (origin.value === 'finishProductStore') {
