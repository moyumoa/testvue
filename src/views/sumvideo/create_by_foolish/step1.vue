<template>
  <div class="step1_page" v-loading="pageLoading">
    <div class="page_left">
      ...
      <template v-if="!formInfo.audioProcessMode">
        <div class="choose_warp">
          <div class="tip_warp">请选择素材音频的处理方式</div>
          <div class="btn_warp">
            <el-button
              class="change_btn"
              type="primary"
      ...
      <div class="content_box" v-if="!!formInfo.audioProcessMode">
            ...
            <el-radio-group v-model="formInfo.composeMode" @change="changeHandelVideoRadio">
                ...
                <el-tooltip placement="top">
                  <template #content>
                    <p style="max-width: 260px">
                      对素材时长范围有要求，合成出所有视频的时长在选择的固定时长范围内
                    </p>
                  </template>
                  <img
              ...
              <el-radio :label="2">
                ...
                <el-tooltip placement="top">
                  <template #content>
                    <p style="max-width: 260px">
                      对素材时长范围无要求，根据选择使用的素材数量从素材池内随机挑选后进行排列组合合成视频
                    </p>
                  </template>
                  <img
                    src="https://tagvv-1256030678.file.myqcloud.com/2024082277ofo.png"
        ...
        <div class="video_setting_warp">
          ...
          <div
            ...
            "
          >
            {{ videoOption.length < 1 ? '请先至少上传2个视频素材' : validList.composeMode.tip }}
          </div>
        </div>
      </div>
      <!-- 保留原音的BGM -->
      ...
      <div class="right_bottom" v-loading="materialLoading || materialOperation.uploadLoading">
        <div class="bottom_content" ref="rightContent">
          ...
          >
            ...
            <div>每次上传最多选择50个素材，视频支持：mp4/mov，视频不超过500M</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <uploadMaterial
      ...
      :limit="50"
      :multiple="true"
      commonErrTip="单个视频不超过500M且单次上传不超过50个素材"
...
const formInfo = ref({
  audioProcessMode: 0, // 素材音频处理方式： 0未选择时页面 1保留视频原音 2不用视频原音
  category: '',
  industry: [],
  sellingPoint: [],
  painPoint: [],
  ...
  activity: [],
  durationDesc: 1,
  enableVideoSound: true, // 素材声音
  resourcesIds: [], // 选中的所有素材IDs
  composeMode: 1, // 视频合成设置 1固定时长 2随机片段
  videoNumList: [], // 组合的素材个数
  bgmConfig: {} // bgm
}) // 左侧填写的信息

const errIdList = ref([]) // 不符合时长的素材ID列表
const materialList = ref([]) // 选中的素材列表
const page = reactive({
  index: 1,
  size: 42,
  total: 0
...
function handelChangeVideoNum() {
  if (formInfo.value.videoNumList.length > 0) {
    ...
    validList.composeMode.tip = '请选择素材使用个数'
  }
}
...
const beforeDet = throttle(function () {
  ...
  const contentText = `从匹配的素材中删除所选素材？<br/><span style="line-height: 30px;font-size:13px;color:#606266">注：不会删除素材库中对应的素材</span>`
  ElMessageBox.confirm(contentText, '删除素材', {
...
function validMaterialFun(key) {
  return new Promise((resolve, reject) => {
    ...
    if (!materialList.value || (materialList.value && materialList.value.length === 0)) {
      validList[key].tip = '请选择素材'
      reject(new Error('请选择素材'))
    }
    // 随机片段不需要进行时长校验
...
const chooseMaterialDialog = ref({
  ...
  dialogTitle: '',
  materialType: 1,
  materialTypeList: [1] // 素材类型 1视频 2图片 3音频 4文本
})
const materialLoading = ref(false) // 加载素材的Loading
...
// 保存选中的素材-拿到选中的素材ID，先存到流水线配置，再通过流水线详情接口获取所有素材ID，根据ID获取对应的分页素材列表
