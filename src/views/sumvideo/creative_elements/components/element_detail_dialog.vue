<template>
  <el-dialog
    ...
    destroy-on-close
    v-model="show"
    :title="`${loading ? '素材' : ELEMENT_TYPE.getCN(info.type) || '未知'}素材`"
    width="560px"
  ...
  >
      ...
      <template v-if="isNotEmpty(info)">
        ...
        <div class="video_content">
          ...
          >
            ...
            <div class="one_info">
              <div class="info_label">有效期至：</div>
              <div class="info_content info_content_flex">
                <div :class="['info_content_text', isExpired ? 'info_content_text__fail' : '']">
                  {{ info.expireTime || '-' }}
                  <span v-if="isExpired">（已过期）</span>
                </div>
                <div @click="editExpireTime" class="info_content_main_text">编辑</div>
              </div>
            </div>
            <div class="one_info">
              <div class="info_label">素材来源：</div>
    ...
    <template #footer v-if="origin != 'mergeDisassembleRecord'">
      <span class="dialog-footer" v-if="!loading">
        ...
        <el-button type="primary" v-if="info.type == 1 && !isExpired" @click="disassemble">拆分</el-button>
      </span>
    </template>
  </el-dialog>
  <!--设置过期时间  -->
...
import ExpiredDialog from './expired_dialog.vue'
    ...
    ElMessageBox.confirm('是否要下载该素材?', '系统提醒', {
      ...
      beforeClose: (action, instance, done) => {
        if (action === 'confirm') {
          ...
          downLoadFile(
            info.value.url,
            (info.value.name || '素材').replace(regex, ''),
            'material_library'
          )
            .then(() => {
...
const isExpired = computed(() => {
  return isNotEmpty(info.value.expireTime) && Date.now() > getDateTimeStamp(info.value.expireTime)
})
// 编辑过期时间
const editExpireTime = () => {
    ...
    .content_info_box {
      ...
      .one_info {
        ...
        .info_content_flex {
          display: flex;
          align-items: center;
          .info_content_text__fail {
            color: #909399;
          }
          .info_content_main_text {
            margin-left: 4px;
            color: #364fcd;
            cursor: pointer;
          }
