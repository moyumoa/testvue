        ...
        <div class="last-num">
          ...
          <div class="num-r" style="width: 696px">
            ...
            <div class="num-r-bottom">
              上传批次号便于查找批量上传的同批{{
                tipsType ? tipsType : origin === 'finishProductStore' ? '成片' : '素材'
              }}，批量进行打标、删除操作
            </div>
          </div>
        </div>
...
function handleBeforeUpload(file, fileList) {
  ...
  if (!allowedTypes.includes(suffix.toLowerCase())) {
    ...
    message({
      ...
      message:
        origin.value === 'mediaResource'
          ? '单个BGM和音效不超过20M，单个字体不超过25M，单个贴纸不超过15M且单次上传不超过50个成片'
          : '单个图片不超过15M，单个视频不超过500M且单次上传不超过50个成片'
    })
  ...
  fileOk(params)
    .then(res => {
      if (res.code === 0) {
        ...
        saveLoading.value = false
        fileLoading.value = false
        message.success(`${tipsType.value || '素材'}批量上传成功`)
