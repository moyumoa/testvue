<template>
  <div class="material_library sumvideo_page_xmy">
    ...
    <div ref="xmSearchRef" class="search_warp">
      ...
      >
        <template v-if="!isShrinking" #searchItem>
          <div class="search_item">
            <label class="item_label">有效状态</label>
            <div class="item_content">
              <el-select
                v-model="otherSearchInfo.expireStatus"
                multiple
                collapse-tags
              ...
              >
                ...
                <!-- 有效期有选择时间这里不能选择 -->
                <el-option
                  @click="expireTips('expireStatus')"
                  label="未设置有效期"
                  :value="0"
                  :disabled="isNotEmpty(otherSearchInfo.expireTime)"
                ></el-option>
              </el-select>
            </div>
          </div>
          <div class="search_item">
            <label class="item_label">有效期</label>
            <div
              :class="{
                item_content: true,
                item_content_disabled: otherSearchInfo.expireStatus.includes(0)
              }"
              @click="expireTips('expireTime')"
            >
              <!-- 有效状态选择未设置有效期 这里不能选择 -->
              <el-date-picker
                :editable="false"
                v-model="otherSearchInfo.expireTime"
                :disabled="otherSearchInfo.expireStatus.includes(0)"
                type="daterange"
                range-separator="至"
        ...
        <template #labelTip>
          <el-tooltip class="item" effect="dark" content="" placement="bottom">
            <template #content>
              <div style="max-width: 400px">开关开启后，素材将在终端开放给员工使用</div>
            </template>
            <img
      ...
      <div class="manage_table" v-loading="!changeFileLoading && tableForm.loading">
        ...
        <div style="position: relative; padding-top: 30px">
          ...
          <div class="batch-operation" v-if="showBatch">
            <el-button @click="openSettingExpired">设置有效期</el-button>
            <el-button style="margin-right: 16px" @click="handleCommandMerge">素材合并</el-button>
            <!-- trigger="click" -->
            ...
            <template v-if="tableForm.listData && tableForm.listData.length > 0">
              ...
              >
                ...
                >
                  <!-- 视频 -->
                  <div
                    :class="{ one_video: true, one_item: true, one_item_mouse: mouseId == item.id }"
                    v-if="item.type === 1"
                  >
                    ...
                    <img
                      v-if="item.source === 6"
                      class="item_xm"
                      src="https://tagvv-1256030678.file.myqcloud.com/20250617b5h8z.png"
                      alt=""
                    />
                    <div
                      class="chose-mask"
                  ...
                  <!-- 图片 -->
                  <div
                    :class="{ one_pic: true, one_item: true, one_item_mouse: mouseId == item.id }"
                  ...
                  <!-- 文本-->
                  <div
                    :class="{ one_text: true, one_item_mouse: mouseId == item.id }"
          ...
          <div class="last-num">
            ...
            <div class="num-r" style="width: 696px">
              ...
              <div class="num-r-bottom">
                上传批次号便于查找批量上传的同批素材，批量进行打标、删除操作
              </div>
            </div>
          </div>
        ...
        <div class="con" v-else-if="showUploadMask.type === 'allMarking'">
          ...
          <div class="batch-label" v-if="batchItem === 2 || batchItem === 1">
            ...
            <div style="width: 90%">
              ...
              <div class="batch-tips">去除所选{{ batchOperationArr.length }}个素材的指定标签</div>
            </div>
          </div>
          <!-- 批量打标 -->
          <div class="batch-label" v-if="batchItem === 4 || batchItem === 3">
            ...
            <div style="width: 90%">
              <batchMarking :batchAbout="batchAbout" @choseOptions="choseOptions" />
              <div class="batch-tips">批量打标{{ batchOperationArr.length }}个素材</div>
            </div>
          </div>
    ...
    <ExpiredDialog
      ...
      :ids="expiredIds"
      @success="onSettingExpiredSuccess"
    ></ExpiredDialog>
  </div>
</template>
...
import ExpiredDialog from './components/expired_dialog.vue'
import {
  getSts,
  getBatchNum,
  batchUpload,
...
const isExpired = expireTime => {
  return Date.now() > getDateTimeStamp(expireTime)
}

// 素材合并
const handleCommandMerge = throttle(function () {
  // 已经过期的数组
  const expiredArr = batchOperationArr.value.filter(x => x.expireTime && isExpired(x.expireTime))
  // 取消选中过期的
  ...
  if (batchOperationArr.value.length < 2) {
    const msg = isNotEmpty(expiredArr)
      ? '已为您过滤不满足条件的过期素材，请最少勾选2个素材'
      : '最少勾选2个素材'
    ...
    return message.warning('素材合并仅支持视频类型')
  } else if (batchOperationArr.value.length > 5) {
    return message.warning('最多支持同时合并5个视频素材')
  }
  // 过滤后满足合并条件
  if (isNotEmpty(expiredArr)) {
    nextTick(() => {
      message.success('已为您过滤不满足条件的过期素材')
    })
  }
  console.log('选择的视频素材', videoArr)
...
function disassembleFun(id, duration, name) {
  console.log('素材拆分 - elementDialog', id)
...
function optionChange(item) {
  // 1 去标选中创意素材 2 去标搜索出的所有创意素材 3 打标选中创意素材 4 打标搜索出的所有创意素材 5 删除选中创意素材 6 删除搜索出的所有创意素材 7移动选中素材 8移动搜索出的所有素材
...
function batchDownLoadFun(removeArr) {
  ...
  ElMessageBox.confirm(
    `已选择 ${removeArr.length} 条素材，${
      removeArr.length > 50 ? '单次下载不超过 50 条，' : ''
    }是否下载${removeArr.length > 50 ? '前 50 条素材' : '选中素材'}？`,
    '批量下载',
    {
...
const resetSearch = throttle(function () {
  ...
  otherSearchInfo.value = {
    expireStatus: [0, 1], // 默认未过期 未设置有效期的
    expireTime: []
  }
  resetTagCloud()
  pageScroll.value?.scrollTo(0, 0)
...
function handleBeforeUpload(file, fileList) {
  ...
  if (!allType.includes(suffix.toLowerCase())) {
    ...
    message({
      ...
      message: '单个图片不超过15M，单个音频不超过20M，单个视频不超过500M且单次上传不超过50个素材'
    })
    // console.log(upload.value, 'upload.value-----------创意素材大小不能超过50M')
    upload.value.clearFiles()
    tableForm.loading = false
    fileLoading.value = false
    return false
...
function beoreOpenChange(item) {
  ...
  return new Promise(resolve => {
    mouseId.value = item.id
    ElMessageBox.confirm(`确认${item.isOpenMaterial ? '取消设置' : '设置'}为开放素材吗？`, '提示', {
      ...
      .then(() => {
        save({
          id: item.id,
          isOpenMaterial: item.isOpenMaterial === 1 ? 0 : 1
        })
          .then(res => {
            if (res.code === 0) {
              ElMessage({
...
const expiredIds = ref([])

// 打开有效期弹窗
function openSettingExpired() {
  if (isEmpty(batchOperationArr.value)) {
    ElMessage({
      message: '您未选择素材',
      type: 'warning'
    })
  } else {
    expiredIds.value = batchOperationArr.value.map(i => i.id)
    nextTick(() => {
...
// 有效期选项提示
const expireTips = key => {
  // 有效状态选项被禁用提示
  if (key === 'expireStatus' && isNotEmpty(otherSearchInfo.value.expireTime)) {
    ElMessage({
      message: '请清空有效期时间后选择',
      type: 'warning'
    })
  ...
  // 有效期被禁用提示
  if (key === 'expireTime' && otherSearchInfo.value.expireStatus.includes(0)) {
    ElMessage({
      message: '请去掉有效状态内‘未设置有效期’选项后使用',
      type: 'warning'
    })
  }
}
