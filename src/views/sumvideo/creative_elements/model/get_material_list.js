...
import { throttle, getCWidth, isNotEmpty, dateFormat } from '@/utils/tools.js'
export function pageList() {
  ...
  function getList() {
    return new Promise((resolve, reject) => {
      ...
      const params = {
        isOpenMaterial: data.isOpenMaterial === null ? null : data.isOpenMaterial,
        ...
        subSource: [1, 0], // 限定素材的子来源  1是 素材征集 0是未指定子来源的素材
        // expireStatus 没有传接口 只会返回 未设置和未过期的 用户没有选择这里需要返回全部 所以都加上
        expireStatus: isNotEmpty(otherSearchInfo.value.expireStatus)
          ? otherSearchInfo.value.expireStatus
          : [0, 1, 2],
        expireTimeMin: isNotEmpty(otherSearchInfo.value.expireTime)
          ? dateFormat(otherSearchInfo.value.expireTime[0]) + ' 00:00:00'
          : undefined,
        expireTimeMax: isNotEmpty(otherSearchInfo.value.expireTime)
          ? dateFormat(otherSearchInfo.value.expireTime[1]) + ' 23:59:59'
          : undefined
      }
      // 如果标签有内容 就记录标签查询记录
