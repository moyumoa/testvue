...
const searchTagDialog = reactive({
  ...
  type: null, // 类型
  hideTitleSpan: true,
  subtitle: '带有特殊标识的标签为云素材标签',
  canDelete: true,
  source: 0, // 0 品牌创建 6 星麦内置（和素材保持一致）
  chooseTagList: [] // 已选择
})
...
function init() {
  ...
  pageInfo.dynamicPageNo = 1
  pageInfo.staticPageNo = 1
  filterName.value = props.rulesOrigin > 10 ? '模型' : '素材'
