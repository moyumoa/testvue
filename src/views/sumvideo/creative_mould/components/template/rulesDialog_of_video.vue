      ...
      >
        <div class="rules_left" ref="scrollRef">
          ...
          <div class="one_rules" v-if="!isReplace">
            <div class="one_label" style="line-height: 24px">不可替换素材选择：</div>
          ...
          <!-- 长素材处理方式 -->
          <div class="one_rules" v-if="!isReplace">
            <div class="one_label">长素材处理方式：</div>
            <div class="one_content">
              <el-select
...
const $data = reactive({
  ...
  chooseLoading: false, // 从素材库选择的loading
  chooseList: [], // 从素材库选择的视频列表
  listTotal: null // 从素材库选择的视频总数
})
...
function showMaterialAllFun() {
  ...
  materialDialog.value = {
    ...
    mediaType: props.ruleDialog.mediaType, // 实际的素材类型
    materialType: props.ruleDialog.mediaType, // 要显示的素材类型 因为不足2秒的音频是打开文本弹窗
