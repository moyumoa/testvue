<template>
  <div v-loading="pageLoading" class="template_edit">
    ...
    <div class="module_structure" :style="showDel ? 'height:calc(100% - 136px - 16px - 0px)' : ''">
            ...
            <template v-if="list && list.length > 0">
              ...
              >
                ...
                <div class="one_td video_width">
                  <!-- 拆解出来的原视频仅支持预览 -->
                  <div v-if="item.splitCoverUrl" class="has_video" style="margin-right: 8px">
                    <img class="video_cover" :src="item.splitCoverUrl" alt="" />
                    <img
                      @click.stop="
                        previewFun(item.splitUrl, 'video', '查看视频', item.splitCoverUrl || null)
                      "
                      class="video_play"
                      src="https://tagvv-1256030678.file.myqcloud.com/20230304m2yx9.png"
                      alt=""
      ...
      <div class="structure_btn" v-if="!showDel">
        <el-button
          ...
          @click.stop="beforeChangeTemplteFun"
        >
          转换为模板
        </el-button>
        <el-button
    ...
    <!-- 转化为模版 -->
    <ToTemplateDialog
      v-if="showToTemplateDialog"
      v-model="showToTemplateDialog"
      @confirm="changeTemplteFun"
    ></ToTemplateDialog>
  </div>
</template>
<script setup>
...
import ToTemplateDialog from './components/to_template_dialog.vue'
...
const origin = ref(null) // 页面来源 是否是剧本式生成跳转过来的
origin.value = route.query.origin
const pipelineId = ref(null) // 页面来源 是否是剧本式生成跳转过来的 携带流水线id
        ...
        if (res.data != null) {
          records = res.data.map(e => {
            return {
              ...
              clipAsideCopy: e.clipAside,
              splitUrl: e.splitUrl,
              splitCoverUrl: e.splitCoverUrl,
              videoInfo:
...
// 转换为模板
// const toTemplateInfo = reactive({
//   show: false,
//   currentIndex: null, // 导入的哪个分镜的
//   item: {} // 导入分镜
// }) // 添加分镜的弹窗
const showToTemplateDialog = ref(false)
const beforeChangeTemplteFun = throttle(() => {
  validFun(true, 'changeTemplate')
    .then(() => {
      showToTemplateDialog.value = true
    })
    .catch(err => {
...
const changeTemplteFun = throttle(data => {
  validFun(true, 'changeTemplate')
    .then(() => {
...
const toCreateByScriptFun = throttle(() => {
  validFun(false, 'toCreateByScript')
    .then(async () => {
      if (origin.value === 'create_by_script') {
        // 如果是从剧本式生成过来的台词/旁白修改，预估时长发生变化 就需要提示用户
        const needBackTips = isNotEmpty(
          list.value.find(item => {
            return isNotEmpty(item.clipAsideCopy) && (
              getUseTime(item.clipAside, 'max') !== getUseTime(item.clipAsideCopy, 'max') ||
              getUseTime(item.clipAside, 'min') !== getUseTime(item.clipAsideCopy, 'min')
            )
          })
        )
        if (needBackTips) {
          const result = await ElMessageBox.confirm(
            `<div style="color: #323233;margin-bottom: 8px;" >分镜预估时长发生变化，将自动清除不符合要求的原料</div><span>1、时长小于该分镜0.5倍的视频<br/>2、智能倍速处理方式下，超过分镜时长2倍的原料</span>`,
            '系统提示',
            {
              confirmButtonText: '确认',
              showCancelButton: false,
      ...
      saveCreativeScriptClip({
        ...
        step: 1,
        complete: true,
        pipelineId: origin.value === 'create_by_script' ? pipelineId.value : undefined // 剧本式生成的流水线id
      })
        .then(res => {
          if (res.code === 0) {
            // 接口调用成功，打开剧本式生成的页面
...
function fontClips2ServerClips() {
  ...
  records = list.value.map((e, index) => {
    return {
      ...
      clipAside: e.clipAside,
      minDuration:
        origin.value === 'create_by_script' ? getUseTime(e.clipAside, 'min') * 1000 : undefined,
