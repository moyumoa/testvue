...
function getVoiceList() {
  ...
  getVoices({
    ...
    .then(res => {
      ...
      // 如果需要默认信息 就选中第一个
      if (props.needDefault && isEmpty(chooseVoiceList.value)) {
        voiceChangeFun(voiceList.value[0])
      }
    })
    .catch(() => {
      voiceList.value = []
...
const voiceChangeFun = throttle((e, index) => {
  // 单选 点击选中
  if (!props.multiple) {
    voiceList.value.forEach(item => (item.isChoose = false))
    e.isChoose = true
    chooseVoiceList.value = [e]
    return
  }
  // 多选
