<template>
  <div class="auto_create_sub" :class="props.classList.length > 0 ? [...props.classList] : ''">
    <div class="page_top flex_c" :style="{ paddingBottom: pageData.isOpen ? 0 : '12px' }">
      ...
      <div class="top_r flex_c" @click.stop="showSubSetting" v-if="pageData.isOpen && showSetting">
        高级设置
        <img
...
const chooseOne = throttle(function (item) {
  // 单选
  if (!props.multiple) {
    subList.value.forEach(item => (item.isChoose = false))
    item.isChoose = true
    ...
    pageData.choseIds = [deepItem.value]
    pageData.chooseItem = [deepItem]
    beforeEmits()
    return
  }
  // 多选
