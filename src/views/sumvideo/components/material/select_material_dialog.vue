...
const props = defineProps({
  ...
  materialType: Number || String, // 显示时使用的素材类型 1显示视频和图片 2显示图片 3显示音频 4显示文本
  materialTypeList: {
    type: Array,
    default: () => {
      return []
  ...
  fileType: Number, // 文件夹类型 3素材库 4实拍库 5场景库
  fileOrigin: {
    type: String,
    default: ''
  }, // 文件夹来源 默认都是素材 material  实拍图 productShoot 场景图 productScene 媒体资源库 mediaResource
  pipelineId: {
    type: Number,
    default: null // 流水线id
  },
  ...
  resourceType: {
    type: Number,
    default: 0 // 资源类别 0素材库资源 3音色 6普通文本转音频，默认0，仅音频分镜导入文本素材时传6
  },
  isBgm: {
    type: Boolean,
  ...
  replaceModel: Number || String, // 长素材的处理方式 目前只对视频/BGM（小于2s 非BGM）进行校验
...
const selectData = reactive({
  totalNum: 0,
  checkNum: 0, // 已选择数量
  chooseIds: [], // 页面加载出来的，已选中的素材列表，包含了任何条件下的选中
  chooseBgms: [], // 如果是导入bgm就用这个
  chooseInfos: [], // 选中的完整信息
  firstCheckAll: false, // 是否在无内容时，点击了全选，如果是，就是选中所有素材，任何筛选的结果都需要选中
  removeIdList: [], // 无内容全选择，需要排除的ids，只有无筛选内容，并且点击全选后，再点击单个素材的取消选择才将取消选择的素材id放在这里
...
function handleConfirm() {
  ...
  // 如果是模板分镜详情或者一键成片过来的 直接把选中的素材ID列表返回，在分镜规则里会通过ID去调用素材列表接口获取对应的信息
  if (
    origin.value === 'productShoot' ||
    origin.value === 'productScene' ||
...
function importBgmFn() {
  ...
  // 选择素材的ids selectData.chooseBgms
...
function importFn() {
  ...
  const params = {
    ...
    queryParam: listParams.value, // 获取当前列表的入参
    resourceType: resourceType.value // 资源类别 0素材库资源 3音色 6普通文本转音频，默认0，仅音频分镜导入文本素材时传6
  }
  // if (selectData.firstCheckAll) {
  //   console.log('全选')
...
function getEmitsList(getBgm = false) {
  ...
  query
    .then(res => {
      console.log('获取当前分镜/替换项已导入素材返回', res)
      if (res.code === 0) {
        emitsInfo.list = res.data?.records || []
        emitsInfo.total = res.data?.total || 0
        console.log('获取分镜素材用于页面显示', emitsInfo)
