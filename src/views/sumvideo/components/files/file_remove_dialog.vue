...
const props = defineProps({
  removeDialog: {
    ...
    default: () => {
      return {
        onlyRemoveFile: true, // 是否只移动文件夹(将一个文件夹移动到另一个文件夹)  否则就是将成片或作品移动到另一个文件夹
        show: true, // 是否显示弹窗
        isBatchRemove: false, // 是否批量移动成片或作品
        materialIds: [], // 成片或作品ID 移动成片或作品只需传该参
        deleteFileIds: [], // 批量移动成片或作品时 需要获取该成片或作品对应的文件夹IDS
  ...
  origin: {
    type: String,
    default: ''
  }, // material 成片库 finishProduct 成片库 productScene 场景库
  fileType: Number // 文件夹类型 3成片库 4实拍库 5场景库
})
const dialog = ref()
...
const chooseInfo = ref() // 要移动到的文件夹完整信息 主要是文件夹名称 可以及时修改成片或作品上的文件夹名称
function init() {
  console.log('init', onlyRemoveFile.value, props.removeDialog)

  // 如果是移动成片或作品，要先获取该成片或作品的原始文件夹ID
