<template>
  <div class="step_1" v-loading="stepLoading">
    <div class="title_warp">
      <stepTitle
        :currentSteps="1"
        :title="'请为每个分镜导入素材组，并完成旁白角色、字幕等自定义项'"
      ></stepTitle>
      <el-button class="title_warp_btn" type="primary" @click="toEditScript">编辑剧本</el-button>
    </div>
...
import { getQueryVariable, throttle, getUserMenu, getUserParentMenu } from '@/utils/tools.js'
...
const clickNext = ref(false) // 是否点击了下一步，如果每点击，那么素材验证就不要加
...
const toEditScript = () => {
  ...
  if (menu && parentMenu) {
    ...
    sendHrefMessage(`${location.origin}/sumwhy_video/creative_script/detail?id=${props?.ruleForm.chooseTemplateInfo.id}&origin=create_by_script&pipelineId=${pipelineId.value}`)
  }
}
onMounted(() => {
