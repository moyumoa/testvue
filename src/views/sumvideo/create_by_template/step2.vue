<template>
  <div class="step2_page step_page">
    <div class="page_top">
      <stepsTitle :currentSteps="1" title="请为被设置为替换项的分镜导入素材组"></stepsTitle>
    </div>
            ...
            >
              ...
              >
                ...
                >
                  ...
                  <!-- mediaType 素材类型 1：视频 2：音频 3：字幕 4：滤镜 5：特效 6:贴纸 7：转场 8:图片  -->
                  <!-- 视频、图片分镜 -->
                  <videoOrImgMaterial
                    v-if="
...
const replaceModelList = reactive([
  {
    label: '长素材随机选取',
    tip: '从素材中随机选取分镜时长长度的片段',
    id: 0
  },
  {
    label: '选取前段',
    tip: '从素材中选取分镜时长长度的前部分片段',
    id: 1
  },

  {
    label: '智能倍速',
    tip: '对素材进行倍速，超过分镜时长 2 倍的素材不会被使用',
    id: 3
  }
]) // 视频和音频的处理方式列表
...
// 导入素材的相关方法
const materialInfo = ref() // 导入的素材信息
      ...
      .then(res => {
        if (res.code === 0) {
          if (!res.data) {
            ...
            // 切换“智能倍速”的时候，校验该分镜所匹配的素材中，如存在超过该分镜时长2倍的视频素材，则弹窗提示，点击“取消”后，切换不成功，恢复至原有处理方式，点击“刪除”，自动刪除所有超过2倍素材，并将处理方式切换为“智能倍速”
            materialInfo.value[creativeReplaceId].replaceModel = nowChooseModal.value
            ElMessageBox.confirm(
              `该分镜选取素材中存在超过分镜时长2倍素材，暂不支持智能倍速，是否刪除这些素材？<br/><span style="line-height: 30px;font-size:13px;color:#606266">注：不会刪除素材库中对应的素材</span>`,
              '刪除素材',
              {
...
const chooseMaterialDialog = ref({
  ...
  show: false,
  dialogTitle: '',
  materialType: '' // 素材类型 1视频 2图片 3音频 4文本
})
// 打开弹窗
const showChooseMaterialFun = throttle((item, mediaType, useAIText) => {
  console.log('打开导入素材', item.duration, item.itemDuration, mediaType, useAIText)
  if (materialOperation.value.uploadLoading) return
  ...
  chooseMaterialDialog.value = {
    ...
    dialogTitle: '导入素材',
    subtitle:
      mediaType === 1 || (mediaType === 2 && !useAIText) ? `时长小于该分镜0.5倍的素材已过滤` : '',
...
function showMaterialAllFun(item) {
  ...
  materialDialog.value = {
    ...
    mediaType: item.mediaType, // 实际的素材类型
    materialType: materialType, // 要显示的素材类型 因为不足2秒的音频是打开文本弹窗
...
// 查看全部弹窗里点击了删除，所以要更新最新的导入素材信息和数量
...
const delMaterialFun = throttle(function (creativeReplaceId, item) {
  ...
  ElMessageBox.confirm(
    `从该分镜所匹配的素材中删除所选素材？<br/><span style="line-height: 30px;font-size:13px;color:#606266">注：不会刪除素材库中对应的素材</span>`,
    '刪除素材',
    {
      closeOnClickModal: false,
      confirmButtonText: '刪除',
