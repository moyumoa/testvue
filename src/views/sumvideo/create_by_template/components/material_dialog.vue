            ...
            >
              <div class="material_box">
                ...
                <videoOrImgMaterial
                  ...
                  :hideName="hideName"
                />
                <!-- 音频素材 BGM和时长小于2s的非BGM 只能导入音频  其余为文本+AI朗读 -->
                <BGMMaterial
                  :origin="origin"
                  v-if="materialType === 2"
                  :mediaType="materialType"
...
const listData = ref([])
const materialText = ref('') //  要删除的名称 默认素材
...
const delMaterialFun = throttle(function (creativeReplaceId) {
  if (
    ...
    message.warning(`请先选择${materialText.value || '素材'}`)
    return
  }
  ElMessageBox.confirm(
    delMessageBox.value?.content ||
      `从该分镜所匹配的${materialText.value || '素材'}中删除所选${
        materialText.value || '素材'
      }？<br/><span style="line-height: 30px;font-size:13px;color:#606266">注：不会刪除${
        materialText.value || '素材'
      }库中对应的${materialText.value || '素材'}</span>`,
    delMessageBox.value?.title || `刪除${materialText.value || '素材'}`,
    {
