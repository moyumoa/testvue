  ...
  >
    <div class="dialog_content">
      ...
      <div class="upload_box">
        ...
        <!-- <div class="one_upload" v-if="imgList && imgList.length > 51">
          <uploadImage
            :maxSize="15360"
            :index="0"
            origin="sumvideo"
            :useCommonReason="true"
            commonReason="每次最多选择50张图片，每张图片不超过15M"
      ...
      <div class="upload_tip">
        支持批量添加，每次最多添加50个文件，图片支持JPG、JPEG、PNG、WEBP,每张图不超过100M
      </div>
      <!-- 批量打标  -->
      <div class="tag_box">
      ...
      <!-- 文件夹  -->
      <div class="tag_box" v-if="false">
        <div class="tag_label">文件夹选择：</div>
        <div class="tag_content">
          <searchFileDOM
            :fileID="saveFileInfo"
      ...
      <div class="tag_box">
        ...
        <div class="tag_content">
          ...
          <div class="tag_tip">上传批次号便于查找批量上传的同批场景图，批量进行删除操作</div>
        </div>
      </div>
    </div>
    <template #footer>
...
const dialog = ref()
const { show, chooseFileId } = toRefs(props.uploadDialog)
// -- 上传图片
  ...
  imgList.value.forEach(x => {
    if (x.resizeSuccess) {
      params.push({
        ...
        tags: (choseLabelIds.value || []).concat(chooseFileId.value),
        source: 21,
        type: 99,
        url: x.url,
        coverUrl: x.url,
