        ...
        >
          ...
          <div class="batch-operation" v-if="tableBatchInfo.showBatch">
            <el-dropdown popper-class="xm_dropDown" @command="handleCommandRemove">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item command="1">批量移动选中成片</el-dropdown-item>
                  <el-dropdown-item command="2">批量移动搜索出的所有成片</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
            <!-- <el-dropdown  popper-class="xm_dropDown" @command="handleCommandBatch">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item command="1">去标选中成片</el-dropdown-item>
                  <el-dropdown-item command="2">去标搜索出的所有成片</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown> -->
            <el-dropdown popper-class="xm_dropDown" @command="handleCommandAddTag">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item command="1">批量打标选中成片</el-dropdown-item>
                  <el-dropdown-item command="2">批量打标搜索出的所有成片</el-dropdown-item>
                  <el-dropdown-item command="3">批量去标选中成片</el-dropdown-item>
                  <el-dropdown-item command="4">批量去标搜索出的所有成片</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
            <el-dropdown popper-class="xm_dropDown" @command="handleCommandDelete">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item command="1">批量删除选中成片</el-dropdown-item>
                  <el-dropdown-item command="2">批量删除搜索出的所有成片</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
            <el-dropdown popper-class="xm_dropDown" @command="handleCommandDown">
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item command="1">批量下载选中成片</el-dropdown-item>
                  <el-dropdown-item command="2">批量下载搜索出的所有成片</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
            <el-dropdown
            ...
            >
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item command="1">将选中的成片生成二维码</el-dropdown-item>
                  <el-dropdown-item command="2">将搜索出的所有成片生成二维码</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
          </div>
...
const uploadBatchInfo = reactive({
  ...
  title: '', // 标题
  uploadBtnText: '', // 上传成片/成片按钮文案
  cancelBtnText: '', // 取消按钮文案
  confirmBtnText: '', // 提交按钮文案
  littleTips: '', // 上传成片/成片按钮描述
  origin: 'finishProductStore', // 来源 成片库
  type: '', // 成片/成片上传upload 批量操作allMarking
...
function showUploadBatch() {
  ...
  uploadBatchInfo.title = '上传成片'
  uploadBatchInfo.uploadBtnText = '添加成片'
...
function optionChange(item) {
  if (batchChose.includes(item) && tableForm.choseList.length <= 0)
    return message.warning('请先勾选成片')
  const info = {
    batchItem: item, // 1 去标选中创意元素 2 去标搜索出的所有创意元素 3 打标选中创意元素 4 打标搜索出的所有创意元素 5 删除选中创意元素 6 删除搜索出的所有创意元素  7移动选中成片 8移动搜索出的所有成片
    title: '批量操作',
    list: [], // 批量操作的数据
    totalList: 0
  }
  ...
  if (item === 7 || item === 8) {
    ...
    //   message.warning('暂不支持批量下载图文类型的成片')
    // } else {
...
function batchDownLoadFun(type, total) {
  ...
  ElMessageBox.confirm(
    `已选择 ${total} 条成片，${total > 50 ? '单次下载不超过 50 条，' : ''}是否下载${
      total > 50 ? '前 50 条成片' : '选中成片'
    }？<br/><span style="line-height: 30px;font-size:13px;color:#606266">注：图文成片将作为多条成片进行下载</span>`,
    '批量下载',
    {
