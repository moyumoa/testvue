  ...
  <div class="works_detail">
    ...
    <div class="head_info_box">
      ...
      <div class="work_source_box">
        <div class="title">成片来源</div>
        <div class="info_box">
          <span class="label">成片来源：</span>
          <span class="value">
            {{
...
function getUseWay1Selects() {
  ...
  const allList = [
    ...
    { id: 3, label: '成片库下载', value: [2, 21].join(',') },
      ...
      beforeClose: (action, instance, done) => {
        if (action === 'confirm') {
          ...
          downLoadFile(
            ...
            (tableForm.workInfo.name || '成片').replace(regex, ''),
            'material_library'
          )
            .then(() => {
              ElMessage({
...
function beoreOpenChange(item) {
  return new Promise(resolve => {
    ElMessageBox.confirm(`确认${item.isOpen ? '取消设置' : '设置'}为开放成片吗？`, '提示', {
      confirmButtonText: '确认',
      cancelButtonText: '取消',
      type: 'warning',
      customClass: 'drawer_systemBox'
