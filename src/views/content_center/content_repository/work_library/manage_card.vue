...
const tableForm = reactive({
  page: {}, // 分页相关
  tags: [], // 获取到的所有标签的列表
  delTagIds: [], // 要删除的标签id列表(所有成片的标签)
  currentTags: [], // 当前成片标签的数组
  currentTagIds: [], // 要保存的标签id列表(当前成片)
  currentWorkId: '' // 当前添加标签的成片id
})
...
// 点击下载视频的回调
function download(e) {
  ElMessageBox.confirm('是否要下载该成片?', '系统提醒', {
...
watch(
  ...
  val => {
    if (!val) {
      ...
      tableForm.currentTags = [] // 当前成片标签的数组
      tableForm.currentTagIds = [] // 要保存的标签id列表
      tableForm.currentWorkId = '' // 当前添加标签的成片id
    }
  },
  { immediate: true }
)
...
const mouseId = ref(null) // 鼠标挪上去的成片ID
