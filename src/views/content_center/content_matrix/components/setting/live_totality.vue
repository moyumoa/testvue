<template>
  <div class="conditionPage">
    <div :class="errorStu ? 'conditionBox errorBox' : 'conditionBox'">
      <div class="conditionTitle">直播总数:</div>
      <div class="conditionContent">
        <el-input
          class="formItemInput"
          v-model="form.minCount"
          clearable
          placeholder="请输入直播场次数"
    ...
    <el-icon
      ...
      @click="delCondtionList('liveTotality')"
    >
      <i-close />
    </el-icon>
  </div>
...
<script setup>
const emit = defineEmits(['changeLiveNum', 'changeConditionList'])
...
const liveMin = toRef(props, 'liveMin')
const liveMax = toRef(props, 'liveMax')
const form = reactive({
  minCount: undefined, // 最少直播数
  maxCount: undefined // 最多直播数
})
...
function inputChange(stu) {
  ...
  emit('changeLiveNum', form.minCount, form.maxCount)
}
// 关闭筛选条件的回调
...
onMounted(() => {
  form.minCount = liveMin.value
  form.maxCount = liveMax.value
})
</script>
