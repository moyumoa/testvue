...
// type  1 星麦内置  2品牌内置
export function useKeyword(keyType, type) {
  // 表格的查询条件
  const tableForm = reactive({
    loading: false,
    listData: [],
    queryParam: {},
    orderSetting: 'DESC:hint_cnt',
    page: {
      index: 1,
      size: 10,
      total: 0
  ...
  function restList() {
    // 刷新列表选中类型不发生改变
    const typeInfo = tableForm.queryParam.type || type
    tableForm.page.index = 1
    tableForm.queryParam = {}
    tableForm.queryParam.type = typeInfo
    selectValue.value = {
      id: '-1',
      groupName: '全部'
    }
  ...
  function getList() {
    const data = {
      ...
      keyType,
      type: tableForm.queryParam.type || type,
      ...tableForm.queryParam
    }
  ...
  const getKeywordGroup = () => {
    ...
    getKeywordGroupApi(data).then(res => {
      if (res.code === 0) {
  ...
  function exportReportFun() {
    ...
    return exportFileV2(keywordExport, data, '高意向关键词列表导出', 'xlsx')
  }
  return {
    selectValue,
    option,
