  ...
  <div class="dy_wrapper">
    <!--<xm_search>
      <div class="search_item">
        <label class="item_label">小红书号</label>
        <div class="item_content">
          <el-input
            v-model="searchForm.authName"
            placeholder="请输入小红书昵称、小红书号"
            clearable
          />
        </div>
      </div>
      ...
      <div class="search_item">
        ...
        <div class="item_content">
          <el-select
            multiple
            v-model="searchForm.dyAuthStatusList"
            placeholder="请选择授权状态"
            clearable
            style="width: 100%"
          >
            <el-option
              v-for="item in statusListXhsSph"
              :label="item.label"
              :value="item.value"
              :key="item.value"
            />
    ...
    <div class="page_main g_bg mt_16">
      <div class="g_bw">
        <el-button type="primary" @click="addXhsAuth">新增小红书授权</el-button>
        <div class="num_warp">
          <!-- 批量操作 -->
          <div class="one_area" v-show="$data.showBatch">
            ...
            <!-- 操作 -->
            <el-button @click="batchOpFun(1, 3)">授权所选矩阵号</el-button>
            <el-button @click="batchOpFun(0, 3)">解除所选矩阵号授权</el-button>
          </div>
      ...
      >
        <el-table-column label="矩阵号" prop="minProgram" min-width="200px">
            ...
            <div class="table_photo">
              ...
              <div class="table_photo_right">
                <div class="photo_right_top">
                  ...
                  <img
                    v-if="row.platform == 3"
                    src="@/assets/images/content_center/xhs.png"
                    alt
                    class="platform_img"
                  />
                </div>
                <div class="account_id">
                  <span>小红书号：{{ row.orginUid || '--' }}</span>
                </div>
                <div class="photo_right_top">
        ...
        <el-table-column label="类型" min-width="150px">
          <template #default="{ row }">
            <span>
              {{ customerUserType.getCN(row.customerUserType || 'COMMON') }}
            </span>
          </template>
        </el-table-column>
        ...
        <el-table-column min-width="150px">
          ...
          <template #default="{ row }">
            <div style="display: flex; align-items: center">
              <span :style="{ color: row.xhsAuth == 2 ? '#f85151' : '' }">
                {{ getEmpowerStatus(row.xhsAuth) || '--' }}
              </span>
              <el-tooltip placement="top" effect="dark" :style="{ 'margin-left': '4px' }">
                <template #content>
                  <div v-if="row.xhsAuth !== 0">
                    <div>授权时间：{{ row.xhsAuthValidDateTime }}</div>
                    <!-- 上一次失效时间 授权失效才显示 -->
                    <div v-if="row.xhsAuth == 2">
                      上一次失效时间：{{ row.xhsAuthInvalidDateTime || '--' }}
                    </div>
                    <!-- 累计有效授权时长 -->
                    <div>
                      累计有效授权时长：{{
                    ...
                    <div v-if="row.xhsAuth !== 1" style="max-width: 300px">
                      失效原因：{{ failReason(row.xhsAuthInvalidType) || '--' }}
                    </div>
                  </div>
                </template>
                <img
          ...
          <template #header>
            <div class="column_header">
              ...
              <el-tooltip
                popper-class="tooltip-class"
                placement="top"
                content="数据停止更新倒计时90天内仅可获取账号评论数及点赞数，超期将停止所有数据更新"
                :style="{ 'margin-left': '4px' }"
              >
                <img
                  src="@/assets/images/operate/tip_line.png"
          ...
          <template #default="{ row }">
            <span
              :style="{
                color:
                  row.dyAuth === 2 &&
                  xhsAuthInValidTime2RemainDay(row.dyAuth, row.dyAuthInvalidDateTime) >= 84
                    ? '#f85151'
                    : ''
              }"
            >
              {{
                xhsAuthInValidTime2RemainDay(row.dyAuth, row.dyAuthInvalidDateTime)
                  ? xhsAuthInValidTime2RemainDay(row.dyAuth, row.dyAuthInvalidDateTime) > 90
                    ? '已停止更新'
                    : '剩余' +
                      (90 - xhsAuthInValidTime2RemainDay(row.dyAuth, row.dyAuthInvalidDateTime)) +
                      '天停止更新'
                  : '正常更新'
              }}
            </span>
        ...
        <!--        <el-table-column label="数据停止更新倒计时" min-width="150px">-->
        <!--          <template #default="{ row }">-->
        <!--            <span>{{ row.dyAuthInvalidDateTime || '&#45;&#45;' }}</span>-->
        <!--          </template>-->
        <!--        </el-table-column>-->
        ...
        <!--  fixed="right" -->
        <el-table-column label="操作" align="right" width="300px" fixed="right">
          <template #default="{ row }">
            <div class="operate_box">
              <el-link type="primary" class="ml_16">
                <span v-if="row.dyAuth === 1" @click="cancelAuth(3, row)">解除授权</span>
                <span v-if="row.dyAuth === 2" @click="reAuth(3, row)">重新授权</span>
              </el-link>
              ...
              <el-dropdown>
                ...
                <template #dropdown>
                  <el-dropdown-menu>
                    ...
                    <el-dropdown-item v-if="row.xhsAuth != 1">
    ...
    <!-- 授权弹窗 -->
    <authDOM
      v-if="state.xhsAuthDialog.show"
      :authDialog="state.xhsAuthDialog"
      @updateFun="updateFun"
    />
    <!--取消授权弹框  -->
    <CancelAuthDialog
...
import { customerUserType } from '@/utils/dictionary/content-center.js'
import authDOM from './components/authXHS_dialog.vue'
...
// function xhsAuthInValidTime2RemainDay(authStatus, time) {
//   if (authStatus === 1) {
//     return null
//   } else {
//     const date = new Date(time)
//     const inValidTime = date.getTime()
//     const now = new Date().getTime()
//     const interval = now - inValidTime
//     return Math.ceil(interval / (24 * 3600000))
//   }
// }
// 匹配授权状态
...
function updateFun(isRefresh) {
  ...
  state.xhsAuthDialog.show = false
}

function auth() {
  // 如果是重新授权，就掉update接口更新这个矩阵号的状态
  if (state.costDialog.refreshAuth) {
    // 如果是星力值不足导致的，直接调接口该状态，否则出二维码弹窗 ，直接调接口该状态，否则出二维码弹窗
    if (state.costDialog.refreshAccountInfo.xhsAuthInvalidType === 9) {
