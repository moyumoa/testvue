  ...
  <div>
    ...
    >
      <div class="invite_code">
        <div class="code">
          <p style="margin-bottom: 8px">请使用小红书扫码加入</p>
...
<script setup>
import { getXhsAuthUrl, getXhsAuthUrlStatus } from '@/api/system_setting/authorization'
...
const setTimer = () => {
  timer.value = setInterval(() => {
    if (count.value === 0) {
      ...
      count.value--
      if (count.value % 3 === 0) {
        getXhsAuthUrlStatus({ authNo: authNo.value, qrid: qrid.value }).then(res => {
          if (res.code === 0) {
            // status 1登录成功 2二维码已生成 3 扫码成功 4二维码失效
            // saveStatus   1待授权 2授权成功 3授权失败
            if (res.data.status === '1' && res.data.saveStatus === 2) {
...
const getAuthReq = throttle(() => {
  ...
  codeUrl.value = ''
  scan.value = false
  getXhsAuthUrl({ source: 2 })
    .then(res => {
      if (res.code === 0) {
        const data = res.data
        codeUrl.value = data.authImageBase64 || '' // 授权图片地址
