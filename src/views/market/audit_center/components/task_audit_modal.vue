  ...
  <div>
    ...
    >
      <section class="task">
        <div class="left_page">
          <section class="taskDetail" v-if="taskData.taskInfo.id">
            <div class="to_btn">
              <p class="taskName">
                {{ taskData.taskInfo.name }}
              </p>
              <el-link type="primary" style="line-height: 20px" @click="toTaskDetail">
                任务详情 >>
              </el-link>
            </div>
            <!-- <div class="taskTime">
            ...
            <div class="taskInfo">
              <ul>
                ...
                <li>
                  <p>任务类型：</p>
                  <p>{{ TASK_TYPE.getCN(taskData.taskInfo.type) }}任务</p>
                </li>
                <li>
                  <p>任务归属部门：</p>
                  <p>{{ taskData.taskOtherInfo.departments || '-' }}</p>
                </li>
                <li>
                  <p>任务分发：</p>
                  <!-- KOC任务只显示人数 -->
                  <p v-if="taskData.taskInfo.taskGroup == 3">
                    {{ taskData.taskOtherInfo.expectReleaseNums || 0 }}人
                  </p>
                  <p v-else>
                    {{ TASK_DELIVER_TYPE.getCN(taskData.taskInfo.deliverType) }} | 约{{
                      taskData.taskOtherInfo.expectReleaseNums || 0
                    }}人
                  </p>
                </li>
                ...
                <template v-if="taskData.taskInfo.type == TASK_TYPE.get('collection')">
                  <template v-if="collectType == 1">
                    ...
                    <li>
                      <p>视频长度：</p>
                      <p>{{ videoDurationRange[0] }} - {{ videoDurationRange[1] }}秒</p>
                    </li>
                  </template>
                  <template v-else>
                    <li>
                ...
                <li
                  v-if="
                    taskData.taskInfo.type != TASK_TYPE.get('collection') &&
                    taskData.taskInfo.type != TASK_TYPE.get('study')
                  "
                >
                  <p>任务要求：</p>
                ...
                <li>
                  ...
                  <p
                    ...
                    "
                  >
                    {{ taskData.taskOtherInfo.giveWayCountCoin || 0 }}
                    <el-tooltip
                      popper-class="tooltip-class"
                      content="一个员工完成所有指标时得到的金币数"
                      placement="top-start"
                    >
                      <img
                        src="@/assets/images/operate/tip_line.png"
                        style="width: 12px; height: 12px; margin-left: 8px; margin-top: -2px"
                      />
                    </el-tooltip>
                  </p>
                </li>
          ...
          <section class="taskTarget">
            ...
            <div class="videoTarget">
              ...
              <div
                class="end"
                v-if="taskData.multiJoinTargetReward && taskData.multiJoinTargetReward.targetReward"
              >
                <p style="display: flex">
                  重复参与
                  <span>{{ taskData.multiJoinTargetReward.targetValue }}</span>
                  次：
                </p>
                <p>
                  奖励
                  <span>{{ taskData.multiJoinTargetReward.targetReward }}</span>
                  金币
                </p>
              </div>
            </div>
        ...
        >
          ...
          <div class="block">
            ...
            <div>
              ...
              <div>
                {{ taskData.taskOtherInfo.auditorName }}
                <div
                  class="xm_depart"
                  style="display: inline-block"
                  v-if="
                    taskData.taskOtherInfo.staffStatus && taskData.taskOtherInfo.staffStatus == 2
                  "
                ></div>
              </div>
            </div>
      ...
      <section class="footer" v-if="taskData.taskOtherInfo.status === 3">
        ...
        <!-- 审核通过后自动开启的，就会提示 -->
        <div
          v-if="query.auditResult == 1 && taskData.taskInfo.isAutoStart === 1"
          style="color: #909399; font-size: 12px; padding: 6px 0 0 68px"
        >
          注：该任务审核通过后将自动开启
        </div>
...
const { query, updateTask } = pageList(emits, props, true)
...
const collectType = computed(() => {
  return queryCkeyOfCvalue('collect_type')
})
const videoDurationRange = computed(() => {
  const rangeStr = queryCkeyOfCvalue('video_duration_range')
  if (rangeStr) {
...
function toTaskDetail() {
  window.open(
    getCreateTaskPath(taskData.value.taskInfo.type) +
      '?id=' +
      taskData.value.taskInfo.id +
      '&disEdit=1',
    '_blank'
  )
}
