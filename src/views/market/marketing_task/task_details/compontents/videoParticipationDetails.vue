  ...
  <div>
    ...
    <div class="page_main mt_16">
      ...
      >
        ...
        >
          <template v-slot="{ row }">
            <div>{{ wanSliceFormat(row.completedNumber || 0) }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="play_count_target" label="播放数" min-width="120">
          <template v-slot="{ row }">
            <div>{{ wanSliceFormat(row.playCountTarget || 0) }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="digg_count_target" label="点赞数" min-width="120">
          <template v-slot="{ row }">
            <div>{{ wanSliceFormat(row.diggCountTarget || 0) }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="comment_count_target" label="评论数" min-width="120">
          <template v-slot="{ row }">
            <div>{{ wanSliceFormat(row.commentCountTarget || 0) }}</div>
          </template>
        </el-table-column>
        <el-table-column prop="share_count_target" label="分享数" min-width="120">
          <template v-slot="{ row }">
            <div>{{ wanSliceFormat(row.shareCountTarget || 0) }}</div>
          </template>
        </el-table-column>
        <!-- <el-table-column prop="share_count_target" label="留资数" min-width="120">
          <template v-slot="{ row }">
