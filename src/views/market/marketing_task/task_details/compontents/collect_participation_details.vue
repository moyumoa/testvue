  ...
  <div>
    ...
    <div class="page_main mt_16">
      ...
      >
        ...
        <el-table-column label="成员信息" width="250" fixed="left">
          <template v-slot="{ row }">
        ...
        <el-table-column prop="joinTaskTime" label="参与时间" min-width="110">
          <template v-slot="{ row }">
            <div>{{ row.joinTaskTime || '-' }}</div>
          </template>
        </el-table-column>
        <el-table-column :label="collectType == 1 ? '征集视频数' : '征集素材数'" min-width="120">
          <template v-slot="{ row }">
            {{ wanSliceFormat(row.collectUploadCountTarget || 0) }}
          </template>
        </el-table-column>
