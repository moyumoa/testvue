  ...
  function transTaskData(_taskInfo, copyId) {
    ...
    if (_taskInfo && _taskInfo.infoConfigs && _taskInfo.infoConfigs.length > 0) {
      ...
      if (
        ...
        // 需要话题的平台 加在通用挂在配置后面 这个只有复制任务才加，因为老数据不显示
        if (copyId) {
          const showTopicNeedPlatform = _taskInfo.infoConfigs[infoConfigIndex].children.findIndex(
            x => x.ckey === 'topic_need_platform'
          )
          if (showTopicNeedPlatform === -1) {
            const videoCommonMountIndex = _taskInfo.infoConfigs[infoConfigIndex].children.findIndex(
              x => x.ckey === 'video_common_mount'
            )
            // 不存在就放在视频发布方式之前
            _taskInfo.infoConfigs[infoConfigIndex].children.splice(videoCommonMountIndex - 1, 0, {
              ckey: 'topic_need_platform',
              cvalue: '',
              name: '需要话题的平台'
            })
          }
