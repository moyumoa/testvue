  ...
  <div class="setting_block">
    ...
    <div class="setting_main">
      <el-form-item class="tag_formItem" label="需携带话题：" style="flex: 1">
        ...
        />
        <!-- 如何设置了话题，显示对应平台是否作为校验项 如果是老数据点击编辑和详情不显示 -->
        <div
          class="top_platform"
          v-if="isNotEmpty(formData.topicList) && showTopicPlatform && isNotEmpty(platformList)"
        >
          <el-checkbox-group
            v-model="formData.topicNeedPlatform"
            :disabled="disabled"
            style="height: unset"
          >
            <el-checkbox v-for="(a, b) in platformList" :key="b" :label="a">
              {{ PLATFORM.getCN(a) }}
            </el-checkbox>
          </el-checkbox-group>
          <div class="top_platform_tip">勾选的平台会将话题作为任务参与成功的判断条件</div>
        </div>
      </el-form-item>
...
function updateTask() {
  ...
  videoConfig.children.forEach(config => {
    ...
    // 设置需要话题的平台
    if (config.ckey === 'topic_need_platform') {
      console.log('需要话题的平台', formData.topicNeedPlatform, cvalue.topicList)
      config.cvalue = isNotEmpty(cvalue.topicList) ? formData.topicNeedPlatform.join(',') : ''
    }
...
const showTopicPlatform = ref(false) // 是否显示选择话题平台 老数据不显示
function init() {
  videoConfig.children.forEach(config => {
    ...
    if (config.ckey === 'video_publish_platform' && isNotEmpty(config.cvalue)) {
      ...
      if ([1, 6].includes(props.taskType)) {
        ...
        platformChange(config.cvalue.split(','), !props.isEdit)
      }
    }
    // 获取需要话题的平台
    if (config.ckey === 'topic_need_platform') {
      showTopicPlatform.value = true
      if (isNotEmpty(config.cvalue)) {
        console.log('设置需要话题的平台', config.cvalue)
        formData.topicNeedPlatform = config.cvalue.split(',').map(Number)
      } else {
        if (props.isEdit) {
          formData.topicNeedPlatform = []
        }
      }
    }
...
function platformChange(platform, setChoose = true) {
  // 视频号、小红书不显示
  const list = (platform || [])
    .filter(x => ['1', '4'].includes(x))
    .map(Number)
    .sort()
  if (setChoose) {
    const added = list.filter(item => !platformList.value.includes(item))
    const removed = platformList.value.filter(item => !list.includes(item))
    console.log('新增的/删除', added, removed)
    const chooseList = [...formData.topicNeedPlatform, ...added]

    formData.topicNeedPlatform = chooseList.filter(item => !removed.includes(item))
  }
  platformList.value = list
  console.log('pl1', list, setChoose)
}
