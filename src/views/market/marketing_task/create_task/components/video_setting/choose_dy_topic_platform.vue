  ...
  <div>
    ...
    >
      ...
      <div
        class=""
        :style="{
          width: `${props.width ? props.width : props.origin === 'task' ? '416px' : '360px'}`
        }"
...
import { searchDyTopic, listHistory } from '@/api/market/task'
      ...
      if (res.code === 0) {
        ...
        if (searchType === 1) {
          resData = (res.data.records || []).map(x => {
            if (x.extra) {
              const extraInfo = JSON.parse(x.extra)
              return {
                ...extraInfo,
                id: extraInfo.cid,
                name: extraInfo.cha_name,
                viewNum: extraInfo.view_count
              }
            } else {
              return ''
            }
        ...
        if (searchParam.pageNumber === 1) {
          ...
          searchList.value = searchType === 1 ? resData : res.data || []
          searchParam.total = searchType === 1 ? res.data.total : (res.data || []).length
        } else {
