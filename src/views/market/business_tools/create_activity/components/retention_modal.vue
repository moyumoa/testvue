      ...
      <div class="content" v-loading="initLoading">
        ...
        <div class="table_box">
          <div class="table_th">
            ...
            <div class="th4">是否必填</div>
          </div>
          <template v-if="list.length > 0">
            <draggable v-model="list" group="retention_good" item-key="id" animation="300">
              <template #item="{ element, index }">
                <div class="table_content">
                  <div class="th1 move_text">
                    <img class="move_img" src="@/assets/images/edit.png" alt="" />
                  </div>
                  <div class="th2">
                    <el-checkbox
                      :disabled="element.configType === 0 || element.configType === 1"
                      v-model="element.configShowConfig"
                      :label="
                        element.configType === 0
                          ? '地理位置'
                          : element.configType === 1
                          ? '手机号'
                          : '文本'
                      "
                    ></el-checkbox>
                  ...
                  <div class="th3">
                    <div v-if="!element.isEdit" class="text_edit">
                      <span>{{ element.configTitle || '--' }}</span>
                      <img
                        v-if="element.configType == 2"
                  ...
                  <div class="th4">
                    ...
                    <el-switch
                      ...
                      v-model="element.configMustConfig"
                      :disabled="element.configType === 0 || element.configType === 1"
                    />
                    <el-link type="danger" @click="delItem(index)" v-if="element.configType == 2">
                      删除
                    </el-link>
                  </div>
                </div>
...
<script setup>
import {
  getListAppointmentConfig,
  saveOrUpdateAppointmentConfig
...
function delItem(index) {
  list.value[index].deleteFlag = 1
  dataList.value.push(list.value[index])
...
function handleConfirm() {
  ...
  const textLen = []
  const leaveTextList = [...list.value, ...dataList.value]
  console.log(leaveTextList)
  list.value.forEach(x => {
    ...
    if (x.configType === 2 && x.configShowConfig) {
  ...
  const data = leaveTextList.map((x, i) => {
    return {
      ...
      id: x.id || null,
      deleteFlag: x.deleteFlag || null
    }
  })
  console.log(data)
  saveOrUpdateAppointmentConfig(data)
    .then(res => {
      if (res.code === 0) {
        message({
          type: 'success',
