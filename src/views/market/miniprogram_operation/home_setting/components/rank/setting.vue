      ...
      <div class="container_content">
        <div class="content1">
          ...
          <el-radio-group v-model="radioForm.post_video" @change="postVideoChange">
            ...
            <el-radio label="2">从成片库中选视频</el-radio>
          </el-radio-group>
        </div>
          ...
          <template v-if="radioForm.post_video == 1">
            <el-tooltip placement="bottom">
              ...
              <div class="tip_tip">
                ...
                />
                拍同款或选择通过模板式生成、流水线生产的0次使用成片视频直接发布
              </div>
            </el-tooltip>
          </template>
      ...
      <el-checkbox-group v-model="checkList" class="group_box" @change="checkChange" v-if="false">
        ...
        <el-checkbox :label="8" disabled>
          ...
          <!-- <div class="post_video_warp" @click.stop>
            <p class="tips">员工可使用模板制作视频或选择模板生产的成片视频发布</p>
            <div class="tops_and_jump">
              模板和成片视频来源：在智能短视频-企业模板中设置为开放状态的模板以及该模板生产的成片视频，当前可用模板数量：
              <span class="template_num">{{ followUpNum || 0 }}</span>
              <div class="jump_btn template_num" @click="toFun">
                前往查看
                <img
...
const subtitleObj = {
  1: '企业开放模板，全员自由创作发视频',
  0: [3].includes(systemVersion.value) && [3].includes(miniViewMode.value)
      ? '开关关闭，常用功能将隐藏“发视频”'
      :'开关关闭，全员营销小程序首页将隐藏"发视频"'
} // 企业版侧重任务模式移动到我的页
const checkList = ref([])
...
const radioForm = ref({
  post_video: '1', // 发视频
  forward_video_config: '' // 从成片库发视频时，转发条件的配置，如用户未设置，要传默认条件
})
