<template>
  <div class="operation-activity">
    <!-- 企业和专业版的简洁模式/侧重任务模式开关隐藏，默认开启 -->
    <title-wrapper
      title=""
      :hasSolt="!([3].includes(systemVersion) && [1, 3].includes(miniViewMode))"
      :subtitle="
        subtitleObj[
          [3].includes(systemVersion) && [1, 3].includes(miniViewMode) ? 1 : materialInfo.switch
        ]
      "
    >
      <template #right>
    ...
    <div class="container">
      <navSetting
        v-if="!([3].includes(systemVersion) && [1, 3].includes(miniViewMode))"
...
const toFun = throttle(() => {
  window.open(`${location.origin}/sumwhy_video/material_library`)
})
</script>
