...
import taskOverviewPreview from './task_overview/preview.vue'
import materialConcisePreview from './material_encyclopedia/concise_preview.vue'
...
// 企业版、专业版的简洁模式和侧重任务模式终端
const configListConcise = {
  'mission:banner': {
    id: 'mission:banner',
    label: 'Banner',
    settingPage: markRaw(bannerSetting),
  ...
  task_overview: {    
    ...
    label: '',
    settingPage: null,
    previewPage: markRaw(taskOverviewPreview),
    infoKey: 'rankMajorInfo',
    infoValue: {
      switch: '0',
      err: '',
  ...
  creation_open: {
    ...
    label: '素材宝典',
    settingPage: markRaw(materialSetting),
    previewPage: markRaw(materialConcisePreview),
    infoKey: 'materialInfo',
    infoValue: {
      switch: '0', // 素材宝典开关
      title: '', // 标题栏
...
console.log("检测使用哪个终端",systemVersion.value,miniViewMode.value)
const configList =
  [3].includes(systemVersion.value) && [1, 3].includes(miniViewMode.value)
    ? configListConcise
    : systemVersion.value === 1 || systemVersion.value === 4
    ? configListMajor
    : configListFirst
