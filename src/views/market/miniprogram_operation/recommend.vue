<template>
  <div class="recommend">
    <title-wrapper title="推荐设置" :subtitle="subtitle">
      <template #right>
        <el-switch v-model="switchValue" @change="onSwitch" />
      </template>
    </title-wrapper>
    <div class="container" v-if="switchValue" v-loading="uploadLoading">
      <div class="button-line">
        <el-button type="primary" @click="showDialog = true">选择任务发布视频</el-button>
        <el-button type="primary" @click="showTemplateDialog = true">选择创意模板视频</el-button>
        <div style="display: inline-block; margin-left: 18px; margin-right: 18px">
          <el-upload
          ...
          >
            <el-button :loading="uploadLoading" type="primary">本地上传视频</el-button>
          </el-upload>
        </div>
          ...
          <div class="video-info">
            ...
            <div class="info-wrapper">
              <el-form>
                ...
                <!-- 创意模板 关联任务-->
                <el-form-item label="关联任务：" v-if="element.source === 1">
                  <div class="line-title" v-if="element.taskName">{{ element.taskName }}</div>
                  <span class="switch-text" @click="openTaskDialog(element)">
                    {{ element.taskName ? '更换' : '选择' }}
                  </span>
                </el-form-item>
...
const configKey = {
  CONFIG_GROUP: 'current_version_recommend',
  SWITCH_KEY: 'current_version_recommend_mini_display',
  LIST_KEY: 'current_version_recommend_list'
}
const subtitleObj = {
  open: '推荐近期任务视频中表现数据较好的精彩视频，为员工提供创作灵感',
  close: '开关关闭，全员营销小程序首页将隐藏“本期推荐”'
}
