    ...
    <div style="padding: 0 16px; box-sizing: border-box">
      <div class="box_title">
        ...
        <el-tooltip
          placement="top"
          :content="
            systemVersion === 4 ? '任务视频的播放数' : '任务视频的播放数+任务视频的直播场观数'
          "
          transfer
          max-width="500"
        >
      ...
      <div class="box_all" v-if="list.length > 0">
        <div class="box_data" v-for="(item, index) in list" :key="index">
          ...
          <div class="head_2">
            <div class="line-hidden1">{{ item.name || '--' }}</div>
          </div>
...
import { getTaskRank } from '@/api/market/overview.js'
