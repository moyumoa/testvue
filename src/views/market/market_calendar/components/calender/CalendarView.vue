    ...
    <div :aria-multiselectable="enableDateSelection" class="cv-weeks">
      ...
      >
        ...
        <div
          class="act-list"
          :style="`           
              height: calc(100% - ${itemTop || '0px'});
            `"
          v-if="getWeekItems(weekStart)"
        >
            ...
            <slot :value="i" :weekStartDate="weekStart" :top="getItemTop(i)" name="item">
              <div
                ...
                :style="`top:${getItemTop(i)};${i.originalItem.style};${
                  chooseActId == i.id ? 'border-color:#8294e7 !important' : ''
                };${i.diyStyle.join(';')}`"
                class="cv-item"
...
export default {
  ...
  props: {
    ...
    itemTop: { type: String, default: '3.6em' },
  ...
  computed: {
    ...
    periodEnd() {
      return this.addDays(
        this.incrementPeriod(this.periodStart, this.displayPeriodUom, this.displayPeriodCount),
        -1
      )
    },
    ...
    periodStartCalendarWeek() {
      ...
      const periodWeekStarts = this.beginningOfWeek(this.periodStart, this.startingDayOfWeek)
    ...
    fixedItems() {
      ...
      return this.items.map(e =>
        self.normalizeItem(e, self.currentHoveredItemId && e.id === self.currentHoveredItemId)
      )
    },
  ...
  methods: {
    ...
    // ******************************
    getStartEnd() {
      // console.log(this.weeksOfPeriod, 'weeksOfPeriod')
      // const start = this.dateFormat(this.weeksOfPeriod[0])
      // const endItem = this.weeksOfPeriod[this.weeksOfPeriod.length - 1]
      // const dayArr = this.daysOfWeek(endItem)
      // const end = this.dateFormat(dayArr[dayArr.length - 1])
      // const obj = {
      //   start,
      //   end
      // }
      // this.$emit('getCell', obj)
      this.$emit('getCell', { start: this.periodStart, end: this.periodEnd })
    },
    ...
    onClickItem(calendarItem, windowEvent) {
      console.log('点击某个活动', calendarItem)
      // calendarItem.id 就是点击活动的id
    ...
    getIncrementedPeriod(count) {
      const newStartDate = this.incrementPeriod(this.periodStart, this.displayPeriodUom, count)
    ...
    findAndSortItemsInDateRange(startDate, endDate) {
      ...
      return this.fixedItems
        .filter(item => item.endDate >= startDate && this.dateOnly(item.startDate) <= endDate, this)
        .sort(this.itemComparer)
    },

    dayHasItems(day) {
      return this.fixedItems.find(d => d.endDate >= day && this.dateOnly(d.startDate) <= day)
    },
    ...
    getWeekItems(weekStart) {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        // 开始时间是否在这周内
        const isStart = ep.startDate >= weekStart
        // 结束时间是否在这周内
        const isEnd =
          ep.endDate <=
          new Date(
            new Date(new Date(weekStart).setDate(new Date(weekStart).getDate() + 6)).setHours(
              23,
              59,
              59
            )
        ...
        if (!isEnd) {
          ep.diyStyle.push(`border-top-right-radius: 0;border-bottom-right-radius: 0;`)
        }
        // 设置样式 begin
        switch (startOffset) {
          case 0:
            if (isStart) {
              ep.diyStyle.push(`margin-left: 7px !important`)
            } else {
              ep.diyStyle.push(`margin-left: 0em !important`)
            }
          ...
          case 6:
            if (isStart) {
              ep.diyStyle.push(` margin-left: calc((100% / 7) * ${startOffset} + 7px) !important`)
            } else {
              ep.diyStyle.push(` margin-left: calc((100% / 7) * ${startOffset}) !important`)
            }
        ...
        switch (span) {
          case 1:
            if (isStart && isEnd) {
              ep.diyStyle.push(` width: calc(((100% - 6px) / 7 - 14px)) !important`)
            } else if (isStart || isEnd) {
              ep.diyStyle.push(` width: calc(((100% - 6px) / 7 - 7px)) !important`)
            } else {
              ep.diyStyle.push(` width: calc(((100% - 6px) / 7 )) !important`)
            }
          ...
          case 7:
            if (isStart && isEnd) {
              ep.diyStyle.push(
                `width: calc(((100% - 6px) / 7) * ${span} + ${span}px - 14px ) !important`
              )
            } else if (isStart || isEnd) {
              ep.diyStyle.push(
                `width: calc(((100% - 6px) / 7) * ${span} + ${span}px - 7px ) !important`
              )
            } else {
              ep.diyStyle.push(`width: calc(((100% - 6px) / 7) * ${span} + ${span}px) !important`)
            }
    ...
    getFormattedTimeRange(e) {
      const startTime = this.formattedTime(e.startDate, this.displayLocale, this.timeFormatOptions)
      let endTime = ''
      ...
      return (
        (startTime !== '' ? `<span class="startTime">${startTime}</span>` : '') +
  ...
  // mounted() {
  //   console.log('看看传过来的事件格式', this.items)
  // }
}
</script>
<!--
