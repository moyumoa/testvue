...
export default {
  methods: {
    ...
    formattedPeriod(startDate, endDate, periodUom, monthNames) {
      ...
      //   periodUom,
      //   monthNames,
      //   'startDate, endDate, periodUom, monthNames'
      // )
      monthNames = (monthNames || []).map((item, index) => {
        return Number(index + 1) + ' 月'
      })
      // console.log(monthNames, 'monthNames monthNames')
    ...
    getFormattedWeekdayNames(locale, format, startingDayOfWeek) {
      ...
      return Array(7)
        .fill()
        .map((_, i) => formatter.format(new Date(2017, 0, (i + 1 + startingDayOfWeek) % 7)))
    },
