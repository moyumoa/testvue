...
export default {
  ...
  methods: {
    ...
    saveUploadState() {
      ...
      console.log("存储状态uploadState", uploadState);
    ...
    async uploadList() {
      ...
      const uploadList = this.list.filter(item => item.status === null && (item.file || item.path));
              ...
              if (!fileInfo.ossUrl) {
                ...
                try {
                  ...
                  if (fileInfo.file) {
                    ...
                    console.log('000',fileInfo.path);
                    
                    fileContent = fs.readFileSync(fileInfo.path);
                    console.log('322',fileContent);
